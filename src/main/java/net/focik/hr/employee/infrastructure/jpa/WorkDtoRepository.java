...
interface WorkDtoRepository extends JpaRepository<WorkDto, WorkId> {
    ...
    @Query(value = "select * from praca where id_pracownika = ?1 AND   data like ?2%",
            nativeQuery = true)
    List<WorkDto> findAllByIdEmployeeAndDate(int idEmployee, String date);

//    @Query(value = "select * from praca where id_pracownika = ?1 AND  data like ?2%",
//            nativeQuery = true)
    //List<WorkDto> findAllByIdEmployeeAndDate(int idEmployee, String dateFrom, String dateTo);
}
